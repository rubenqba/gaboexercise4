...
public class CMYKPixelProcessor implements PixelProcessor {

    private ColorScale scale;
    private Integer width;
    private Integer height;

    public CMYKPixelProcessor(ColorScale scale, Integer width, Integer height) {
        this.scale = scale;
        this.width = width;
        this.height = height;
    }
                ...
                .map(pixel -> {
                    if (FLOAT == scale) {
                        List<Float> components = Stream.of(pixel.split(","))
                                ...
                                .collect(Collectors.toList());
                        return new CMYKColor<>(components.get(0), components.get(1), components.get(2), components.get(3));
                    }
